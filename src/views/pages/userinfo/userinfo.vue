  ...
  <div id="data-list-list-view" class="data-list-container">
    ...
    >
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="BuyingShop">unii</vs-th>
      ...
      <template slot-scope="{ data }">
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <div @change="permission(tr.key, tr.unii)">
                <vs-switch
                  color="success"
                  :id="`switch_${tr.key}`"
                  v-model="switch2"
  ...
  methods: {
    ...
    gettype() {
      if (this.$route.query.typeuser === "p") {
        ...
        return "UniiTruck_C";
      } else if (this.$route.query.typeuser === "utuc") {
        return "UniiTruck_UC";
      } else {
        return "Customer";
      }
    ...
    async permission(key, unii) {
      let id = document.getElementById(`switch_${key}`);

      const type = await this.gettype();
      // alert(role);
      if (unii == undefined || unii == false) {
        // alert("true");
        this.$vs.loading();
        await userRef
          .child(type)
    ...
    async checkData(arr) {
      ...
      arr.forEach(item => {
        ...
        if (item.unii == true && !this.switch2.includes(item.key)) {
